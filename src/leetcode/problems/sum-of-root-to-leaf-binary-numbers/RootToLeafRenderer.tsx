...
export function RootToLeafRenderer({ step }: RendererProps<RootToLeafData>) {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-6">
      ...
      <div className="flex items-center gap-3 text-sm">
        {value !== null && <span className="rounded-md border border-role-current px-3 py-1">path value = <b className="tabular-nums">{value}</b></span>}
        <span className="rounded-md border px-3 py-1">sum = <b className="tabular-nums">{answer ?? sum}</b></span>
      </div>

      {leafValues.length > 0 && (
        <div className="text-xs text-muted-foreground">leaves: {leafValues.join(" + ")} = {leafValues.reduce((a, b) => a + b, 0)}</div>
      )}
