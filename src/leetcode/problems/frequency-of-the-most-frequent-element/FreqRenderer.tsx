import type { RendererProps } from "@/core/types";
import { ArrayCells, Legend, roleLookup } from "@/leetcode/shared/viz";
import type { FreqData } from "./algorithm";

export function FreqRenderer({ step }: RendererProps<FreqData>) {
  const { nums, k, left, right, cost, best } = step.data;
  const roleFor = roleLookup(step.highlights);

  return (
    <div className="flex h-full flex-col gap-5">
      <div className="flex flex-wrap items-center justify-center gap-3 text-sm">
        <span className="text-muted-foreground">budget k</span>
        <span className="rounded-md border border-primary bg-primary/10 px-2 py-0.5 font-semibold tabular-nums text-primary">{k}</span>
        <span className="text-muted-foreground">level cost</span>
        <span className={`rounded-md border px-2 py-0.5 font-semibold tabular-nums ${cost <= k ? "border-role-sorted" : "border-role-swapped"}`}>{cost}</span>
        <span className="text-muted-foreground">best freq</span>
        <span className="rounded-md border border-role-target bg-role-target/10 px-2 py-0.5 font-semibold tabular-nums text-role-target">{best}</span>
      </div>

      <ArrayCells values={nums} roleFor={(idx) => roleFor(idx)} topLabel={(idx) => (idx === left && idx === right ? "L/R" : idx === left ? "L" : idx === right ? "R" : "")} />
      <p className="text-center text-xs text-muted-foreground">Sorted; cost = max × windowSize − windowSum.</p>

      <Legend
        items={[
          { role: "active", label: "Window" },
          { role: "current", label: "Right edge" },
          { role: "swapped", label: "Dropped" },
          { role: "target", label: "Affordable window" },
        ]}
      />
    </div>
  );
}
